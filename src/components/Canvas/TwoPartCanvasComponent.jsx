...
const TwoPartCanvasComponent = ({ width, height }) => {
  ...
  useEffect(() => {
    ...
    const outerY = (canvasHeight - height) / 2 + borderWidth / 2;
    const outerWidth = width + 16; // Adjusted width
    ...
    const startY = (canvasHeight - height) / 2 + spacing / 2; // Start Y coordinate
    for (let i = 0; i < numRectangles; i++) {
      const rectX = startX + i * (rectangleWidth + spacing); // Adding spacing
      context.strokeRect(rectX, startY, rectangleWidth, height - spacing); // Adjusted height

      // Add number to the bottom right corner of each rectangle
      const numberX = rectX + rectangleWidth - 15;
      const numberY = startY + height - spacing - 15;
